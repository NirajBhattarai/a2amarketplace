# =============================================================================
# agents/carbon_credit_agent/task_manager.py
# =============================================================================
# 🎯 Purpose:
# Connects the CarbonCreditAgent class to the Agent-to-Agent (A2A) protocol by
# handling incoming JSON-RPC "tasks/send" requests. It:
# 1. Receives a SendTaskRequest model
# 2. Stores the user message in memory
# 3. Calls CarbonCreditAgent.invoke() to asynchronously process carbon credit requests
...
# The core business logic: CarbonCreditAgent with an async invoke() method
...
class CarbonCreditTaskManager(InMemoryTaskManager):
    """
    🧩 TaskManager for CarbonCreditAgent:

    - Inherits storage, upsert_task, and locking from InMemoryTaskManager
    - Overrides on_send_task() to:
      * save the incoming message
      * call the CarbonCreditAgent.invoke() to process carbon credit requests
      * update the task status and history
      * wrap and return the result as SendTaskResponse

    Note:
    - CarbonCreditAgent.invoke() is asynchronous, but on_send_task()
      itself is also defined as async, so we await internal calls.
    """
    def __init__(self, agent: CarbonCreditAgent):
        """
        Initialize the TaskManager with a CarbonCreditAgent instance.

        Args:
            agent (CarbonCreditAgent): The core logic handler that knows how to
                                      process carbon credit negotiation requests.
        """
        # Call the parent constructor to set up self.tasks and self.lock
        super().__init__()
        # Store a reference to our CarbonCreditAgent for later use
        self.agent = agent
    ...
    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        """
        Handle a new carbon credit negotiation task:

        1. Store the incoming user message in memory (or update existing task)
        2. Extract the user's text for processing
        3. Call CarbonCreditAgent.invoke() to process the carbon credit request
        ...
        logger.info(f"CarbonCreditTaskManager received task {request.params.id}")
        ...
        # Step 3: Call the CarbonCreditAgent to process the carbon credit request.
        # Since CarbonCreditAgent.invoke() is an async function,
        # await it to get the returned string.
        carbon_credit_response = await self.agent.invoke(
            user_text,
        ...
        reply_message = Message(
            ...
            parts=[TextPart(text=carbon_credit_response)]  # The agent's reply text
        )
