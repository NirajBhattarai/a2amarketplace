# =============================================================================
# agents/wallet_balance_agent/task_manager.py
# =============================================================================
# 🎯 Purpose:
# Connects the WalletBalanceAgent class to the Agent-to-Agent (A2A) protocol by
# handling incoming JSON-RPC "tasks/send" requests. It:
# 1. Receives a SendTaskRequest model
# 2. Stores the user message in memory
# 3. Calls WalletBalanceAgent.invoke() to asynchronously process wallet balance requests
# 4. Updates the task status and appends the response to task history
...
# The core business logic: WalletBalanceAgent with an async invoke() method
...
class WalletBalanceTaskManager(InMemoryTaskManager):
    """
    🧩 TaskManager for WalletBalanceAgent:

    - Inherits storage, upsert_task, and locking from InMemoryTaskManager
    - Overrides on_send_task() to:
      * save the incoming message
      * call the WalletBalanceAgent.invoke() to process wallet balance requests
      * update the task status and history
      * wrap and return the result as SendTaskResponse

    Note:
    - WalletBalanceAgent.invoke() is asynchronous, but on_send_task()
      itself is also defined as async, so we await internal calls.
    """
    def __init__(self, agent: WalletBalanceAgent):
        """
        Initialize the TaskManager with a WalletBalanceAgent instance.

        Args:
            agent (WalletBalanceAgent): The core logic handler that knows how to
                                      process wallet balance requests across networks.
        """
        # Call the parent constructor to set up self.tasks and self.lock
        super().__init__()
        # Store a reference to our WalletBalanceAgent for later use
        self.agent = agent
    ...
    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        logger.info(f"WalletBalanceTaskManager received task {request.params.id}")
        ...
        # Step 3: Call the WalletBalanceAgent to process the wallet balance request.
        # Since WalletBalanceAgent.invoke() is an async function,
        # await it to get the returned string.
        wallet_balance_response = await self.agent.invoke(
            user_text,
        ...
        reply_message = Message(
            ...
            parts=[TextPart(text=wallet_balance_response)]  # The agent's reply text
        )
