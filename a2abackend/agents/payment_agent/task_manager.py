# =============================================================================
# agents/payment_agent/task_manager.py
# =============================================================================
# 🎯 Purpose:
# Connects the PaymentAgent class to the Agent-to-Agent (A2A) protocol by
# handling incoming JSON-RPC "tasks/send" requests. It:
# 1. Receives a SendTaskRequest model
# 2. Stores the user message in memory
# 3. Calls PaymentAgent.invoke() to asynchronously process payment requests
...
# The core business logic: PaymentAgent with an async invoke() method
...
class PaymentTaskManager(InMemoryTaskManager):
    """
    🧩 TaskManager for PaymentAgent:

    - Inherits storage, upsert_task, and locking from InMemoryTaskManager
    - Overrides on_send_task() to:
      * save the incoming message
      * call the PaymentAgent.invoke() to process payment requests
      * update the task status and history
      * wrap and return the result as SendTaskResponse

    Note:
    - PaymentAgent.invoke() is asynchronous, but on_send_task()
      itself is also defined as async, so we await internal calls.
    """
    def __init__(self, agent: PaymentAgent):
        """
        Initialize the TaskManager with a PaymentAgent instance.

        Args:
            agent (PaymentAgent): The core logic handler that knows how to
                                process payment requests across networks.
        """
        # Call the parent constructor to set up self.tasks and self.lock
        super().__init__()
        # Store a reference to our PaymentAgent for later use
        self.agent = agent
    ...
    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        logger.info(f"PaymentTaskManager received task {request.params.id}")
        ...
        # Step 3: Call the PaymentAgent to process the payment request.
        # Since PaymentAgent.invoke() is an async function,
        # await it to get the returned string.
        payment_response = await self.agent.invoke(
            user_text,
        ...
        reply_message = Message(
            ...
            parts=[TextPart(text=payment_response)]  # The agent's reply text
        )
